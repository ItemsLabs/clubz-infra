...
jobs:
  fanclash-api:  # Create infrastructure for services on push to Main branch
    name: fanclash-api
    ...
    steps:
      ...
      - name: EKS Deploy - Service
        uses: Consensys/kubernetes-action@master
        env:
          KUBE_CONFIG_DATA: ${{ env.KUBE_CONFIG_DATA }}
        with:
          args: apply -f deploy/eks/service.yaml --namespace=${{ env.ENV }}
      ...
      - name: EKS Deploy - Ingress
        uses: Consensys/kubernetes-action@master
        env:
          KUBE_CONFIG_DATA: ${{ env.KUBE_CONFIG_DATA }}
        with:
          args: apply -f deploy/eks/ingress_${{ env.ENV }}.yaml --namespace=${{ env.ENV }}